...
const populateUser = (query: any) =>
  query.populate({ path: 'author', model: User, select: '_id firstName lastName clerkId' });
...
export async function getAllImages({ limit = 10, page = 1, searchQuery = '' }) {
  try {
    ...
    const totalImageCount = await Image.find(query).countDocuments();
    const savedImage = await Image.find().countDocuments();

    return {
      data: JSON.parse(JSON.stringify(images)),
      totalPages: Math.ceil(totalImageCount / limit),
      savedImage,
    };
  } catch (error) {
    handleError(error);
